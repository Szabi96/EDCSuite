...
namespace VAGSuite
{
    public class MSA15FileParser : IEDCFileParser
    {
        ...
        private bool FindSVBLSequenceTwo(byte[] allBytes, string filename, SymbolCollection newSymbols, List<CodeBlock> newCodeBlocks)
        {
            ...
            while (found)
            {
                ...
                if (SVBLAddress > 0)
                {
                    ...
                    int[] testValue = Tools.Instance.readdatafromfileasint(filename, (int)shsvbl.Flash_start_address, 1, EDCFileType.MSA15);
                ...
                if (sh.Length == 544)
                {
                    if (sh.X_axis_length == 16 && sh.Y_axis_length == 17)
                    {
                        if (sh.X_axis_ID / 256 == 0xE0 && sh.Y_axis_ID / 256 == 0xC2)
                        {
                            ...
                            //Todo: N146 correction factor for MSA15
